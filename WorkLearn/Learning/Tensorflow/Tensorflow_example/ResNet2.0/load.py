...
class Resnet:
	...
	def deepnn(self, x_input, classes=2):
		...
		with tf.variable_scope('reference') :
			...
			print(x.get_shape())
			assert (x.get_shape() == (x.get_shape()[0], 55, 55, 64))
			...
			logits = tf.layers.dense(x, units=self.classnum, activation=tf.nn.softmax)
	...
	def train(self, X_train, Y_train):
		...
		labels = tf.placeholder(tf.int64, [None, self.classnum])

		logits, keep_prob, train_mode = self.deepnn(tf.reshape(features,[-1,224,224,3]))
		...
		with tf.Session() as sess:
			...
			for i in range(1000):
				X_mini_batch, Y_mini_batch =sess.run([X_train,Y_train]) #mini_batches[np.random.randint(0, len(mini_batches))]
				train_step.run(feed_dict={features: X_mini_batch, labels: Y_mini_batch, keep_prob: 0.5, train_mode: True})
				train_step_out,cross_entropy_out=sess.run([train_step,cross_entropy],feed_dict={features: X_mini_batch, labels: Y_mini_batch, keep_prob: 0.5, train_mode: True})
				print('step %d, training cost %g' % (i, cross_entropy_out))
			coord.request_stop()
			coord.join(threads)
...
reader = kaggleCatDogLoad.ImageNetDataSet("F:/kaggle_cat_dog_dataset/train",32)
reader.get_labels()
