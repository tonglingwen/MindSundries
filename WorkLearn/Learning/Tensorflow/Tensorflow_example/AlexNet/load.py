...
h_pool1=tf.nn.local_response_normalization(tf.nn.max_pool(h_conv1, ksize=[1, 3, 3, 1],strides=[1, 2, 2, 1], padding='SAME'))
...
h_pool2=tf.nn.local_response_normalization(tf.nn.max_pool(h_conv2, ksize=[1, 3, 3, 1],strides=[1, 2, 2, 1], padding='SAME'))
...
w_conv5=tf.Variable(tf.truncated_normal([1,1,384,256], stddev=0.1))
...
h_fullconv=tf.reshape(h_pool5,[-1,4*4*256])
print("h_conv5:",h_conv5.shape)
print("h_pool5:",h_pool5.shape)
print("h_fullconv:",h_fullconv.shape)

w_full6=tf.Variable(tf.truncated_normal([4*4*256,4096], stddev=0.1))
